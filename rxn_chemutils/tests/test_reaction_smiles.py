...
from rxn_chemutils.reaction_smiles import (
    determine_format, ReactionFormat, parse_reaction_smiles, parse_any_reaction_smiles,
    to_reaction_smiles, canonicalize_reaction_smiles
)


def test_determine_format():
    assert determine_format('CC.O>>CCO') is ReactionFormat.STANDARD
    assert determine_format('CC.O.[Na+]~[Cl-]>>CCO') is ReactionFormat.STANDARD_WITH_TILDE
    assert determine_format('CC.O.[Na+].[Cl-]>>CCO |f:2.3|') is ReactionFormat.EXTENDED


def test_parse_any_reaction_smiles():
    assert parse_any_reaction_smiles('CC.O>>CCO') == ReactionEquation(['CC', 'O'], [], ['CCO'])
    assert parse_any_reaction_smiles('CC.O.[Na+]~[Cl-]>>CCO') == ReactionEquation(
        ['CC', 'O', '[Na+].[Cl-]'], [], ['CCO']
    )
    assert parse_any_reaction_smiles('CC.O.[Na+].[Cl-]>>CCO |f:2.3|') == ReactionEquation(
        ['CC', 'O', '[Na+].[Cl-]'], [], ['CCO']
    )
...
def test_parse_reaction_smiles():
    ...
    assert parse_reaction_smiles('CC.O.[Na+]~[Cl-]>>CCO', ReactionFormat.STANDARD_WITH_TILDE
                                 ) == ReactionEquation(['CC', 'O', '[Na+].[Cl-]'], [], ['CCO'])
    assert parse_reaction_smiles('CC.O.[Na+].[Cl-]>>CCO |f:2.3|', ReactionFormat.EXTENDED
                                 ) == ReactionEquation(['CC', 'O', '[Na+].[Cl-]'], [], ['CCO'])
...
def test_to_reaction_smiles():
    ...
    assert to_reaction_smiles(
        reaction_equation, ReactionFormat.STANDARD
    ) == 'CC.O.[Na+].[Cl-]>>CCO'
    assert to_reaction_smiles(
        reaction_equation, ReactionFormat.STANDARD_WITH_TILDE
    ) == 'CC.O.[Na+]~[Cl-]>>CCO'
    assert to_reaction_smiles(
        reaction_equation, ReactionFormat.EXTENDED
