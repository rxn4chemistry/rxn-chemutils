...
def test_sort(chemical_reaction):
    assert str(chemical_reaction.sort()) == "O[Na].[14C]Cl>O>[14C]O.[Na]Cl"


def test_sort_only_reactants(chemical_reaction):
    assert (str(chemical_reaction.sort(
        sort_products=False,
        sort_agents=False)) == "O[Na].[14C]Cl>O>[Na]Cl.[14C]O")


def test_remove_precursors_from_products(dirty_chemical_reaction):
    assert (str(dirty_chemical_reaction.remove_precursors_from_products()) ==
...
def test_remove_none(chemical_reaction):
    chemical_reaction.products.append(None)
    assert len(chemical_reaction.remove_none().products) == 2