...
from rxn_chemutils.multicomponent_smiles import (
    multicomponent_smiles_to_list, list_to_multicomponent_smiles, apply_to_multicomponent_smiles,
...
def test_apply_to_multicomponent_smiles():
    ...
    assert apply_to_multicomponent_smiles('A.B~C', dummy, fragment_bond='~') == 'A1.B~C2'
    assert apply_to_multicomponent_smiles('ABCD.B~C~D', dummy, fragment_bond='~') == 'ABCD1.B~C~D3'
...
def test_canonicalize_multicomponent_smiles():
    ...
    assert canonicalize_multicomponent_smiles('C.CF(C).O', check_valence=False) == 'C.CFC.O'
    with pytest.raises(InvalidSmiles):
...
def test_remove_duplicates_in_multicomponent_smiles():
    ...
    assert remove_duplicates_in_multicomponent_smiles('CC.C(C).(C)(C)') == 'CC.C(C).(C)(C)'
